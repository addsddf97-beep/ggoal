...
음식 이름이나 간단한 아이디어를 입력하면 로컬 Ollama, 로컬 이미지 API, 로컬 TTS API로 유튜브 숏츠용 음식 캐릭터 상황극을 생성하는 MVP입니다.

## 구조

- `apps/api`: Next.js Route Handlers 기반 백엔드 API, Ollama 텍스트 생성, 로컬 이미지 생성, 로컬 TTS, 로컬 파일 저장
...
- `POST /api/images`: 로컬 LlamaGen 이미지 API로 씬별 캐릭터 이미지 생성 후 `apps/api/public/generated/{jobId}`에 저장
